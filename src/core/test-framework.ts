...
export class TestFramework {
  ...
  constructor(page: Page, context: BrowserContext, testInfo: TestInfo) {
    ...
    this.pageObjectBase = new PageObjectBase(page);
    this.flakyDetector = new FlakyTestDetector();
    this.logger = new Logger(`TestFramework-${testInfo.title}`);
  }
  ...
  async setupTest(): Promise<void> {
    try {
      ...
      await this.pageObjectBase.navigateTo();
  ...
  async teardownTest(): Promise<void> {
    try {
      ...
      // Cleanup
      await this.pageObjectBase.cleanup();
  ...
  private async setupAllureMetadata(): Promise<void> {
    ...
    // Add test story
    allure.story('Web Application Testing');
 ...
 */
export const test = base.extend<{ testFramework: TestFramework }>({
  testFramework: async ({ page, context }, use, testInfo) => {
    const testFramework = new TestFramework(page, context, testInfo);
    await testFramework.setupTest();
    await use(testFramework);
